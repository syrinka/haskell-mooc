...
firstHalf xs = take (if even len then div len 2 else div len 2 + 1) xs
    where len = length xs
...
capitalize xs = unwords . map (\(x:xs) -> (toUpper x : xs)) . words $ xs
...
sumRights li = sum . map (either (const 0) id) $ li
...
interpreter' x y [] = [] -- eol
interpreter' x y (cmd:cmds) = case cmd of
    "up"    -> interpreter' x (y+1) cmds
